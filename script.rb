...
hotels = File.open('list_hotel').read.split("\n").map do |text|
...
hotels.each do |hotel|
  (0...5).each do |time|
    ...
    break unless (200..299).include?(response.code.to_i)

    response = JSON.parse(response.body)

    response.to_a[1].to_h['data'].to_h['locations'].to_a[0].to_h['reviewListPage'].to_h['reviews'].to_a.each do |review|
      hotel_reviews << {
        user_name: review['userProfile'].to_h['displayName'],
        user_location: review['userProfile'].to_h['hometown'].to_h['location'].to_h['name'],
        trip_date: review['tripInfo'].to_h['stayDate'],
        trip_type: review['tripInfo'].to_h['tripType'],
...
CSV.open('final_result_2.csv', 'w') do |csv|
  csv << ['User name', 'User location', 'Trip date', 'Trip type', 'Hotel name']
  hotel_reviews.each { |review| csv << review.values }
end
